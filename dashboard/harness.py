...
process via QEMU's QMP protocol (JSON-RPC over TCP :45454).

Architecture: see media/writings/dashboard_harness_plan.md.

Run directly:
    python harness.py                 # default port 8080, qmp at localhost:45454
    python harness.py --port 9000     # custom port
    python harness.py --qmp-host 192.168.1.10 --qmp-port 45454
    ...
    def status_payload(self, extra=None):
        ...
        if self._is_refused_connection(base.get("last_error")):
            base["setup_hint"] = (
                ...
                "(not wr.bat) so QEMU binds -qmp tcp:127.0.0.1:45454,server,nowait."
            )
        if extra:
            base.update(extra)
        return base
...
def main():
    ...
    parser.add_argument("--qmp-port", type=int, default=45454, help="QEMU QMP port")
