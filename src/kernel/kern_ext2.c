...
u32 ext2_get_bmap(ext2_inode_t *inode, u32 logical_block) {
    if (!inode || block_size == 0) return 0;
    u32 n = block_size / 4;
    ...
    u32 table[1024]; // 4KB stack buffer
    if (block_size > sizeof(table)) return 0;

    if (logical_block < n) {
        if (inode->block[12] == 0) return 0;
        ext2_read_block(inode->block[12], (u8 *)table);
    ...
    if (logical_block < n * n) {
        ...
        ext2_read_block(inode->block[13], (u8 *)table);
        u32 indirect_block = table[logical_block / n];
    ...
    if (logical_block < n * n * n) {
        ...
        ext2_read_block(t_block, (u8 *)table);
        u32 d_indirect_block = table[logical_block / (n * n)];
        if (d_indirect_block == 0) return 0;
        ext2_read_block(d_indirect_block, (u8 *)table);
        u32 indirect_block = table[(logical_block / n) % n];
        if (indirect_block == 0) return 0;
        ext2_read_block(indirect_block, (u8 *)table);
        return table[logical_block % n];
    }
